...
function largethree(){
  function Image(selector, time) {
    ...
    This.show = 1;
    This.time = time || 1000;
    This.imglist = document.querySelectorAll(selector + ' .imglist li');
    This.btnlist = document.querySelectorAll(selector + ' .btn li');
    for (var i = 0; i < This.btnlist.length; i++) {
        This.btnlist[i].onclick = (function (ind) {
            return function () {
                console.log(ind);
                //隐藏所有图片
                for (var index = 0; index < This.imglist.length; index++) {
                    This.imglist[index].style.display = 'none';
                }
                //改变当前显示的图片下标
                This.show = ind;
                //显示按钮对应的图片
                This.imglist[ind].style.display = 'block';
            }
        })(i);
    }
}
...
Image.prototype.autoplay = function(){
    ...
    setInterval(function(){
        This.btnlist[(++This.show)%(This.imglist.length)].onclick();
    }, This.time)
}
new Image('#mydiv', 1500).autoplay();
}
